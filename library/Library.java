...
public class Library {
    ...
    // DÁN VÀO FILE Library.java (thay thế hàm cũ)
            ...
            if (C.GetId() == clerk_id) {
                for (int j = 0; j < UsersList.size(); j++) {
                    ...
                    if (borrower_id == U.GetId()) {
                        ...
                        // *** PHẦN ĐÃ SỬA LỖI ***
                        int index = 1; // 1. Mặc định index là 1
                        if (!LoanList.isEmpty()) { // 2. KIỂM TRA NẾU LIST KHÔNG RỖNG
                            int size=LoanList.size();
                            Loan L=LoanList.get(size-1); // 3. Chỉ lấy nếu không rỗng
                            index=L.GetLoanId();
                            index=index+1;
                        }
                        Loan LoanObj= new Loan(index); // 4. Tạo Loan mới với index đúng
                        // *** KẾT THÚC SỬA LỖI ***
